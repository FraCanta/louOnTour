...
function getExtension(fileName = "", contentType = "") {
  ...
  if (extensionFromName && MIME_TO_EXTENSION[contentType] === extensionFromName) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ALLOWED_MIME_TYPES.has(contentType)) {
      return res
        .status(400)
        .json({ error: "Formato immagine profilo non supportato." });
    }
    ...
    if (fileBuffer.length > MAX_UPLOAD_SIZE_BYTES) {
      return res
        .status(400)
        .json({ error: "Immagine troppo grande. Limite 4MB." });
    }

    const extension = getExtension(fileName, contentType);
    const filePath = `admin-profiles/${Date.now()}-${Math.random()
      .toString(36)
    ...
    return res.status(400).json({
      error: error.message || "Upload immagine profilo non riuscito.",
    });
  }
}
